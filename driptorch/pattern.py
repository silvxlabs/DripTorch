...
class TemporalPropagator:
    ...
    def forward(self, paths: dict, ignition_crew: IgnitionCrew, heat_delay: float) -> None:
        ...
        Args:
            ...
            heat_delay (float, optional): Time delay between ignition heats.
        Returns:
    ...
    def _delay_heats(self, heat_delay: float) -> None:
        """Offset the time between respective heats

        Args:
            heat_delay (float): Time offset between heats
        """

        # Get the unique heat indecies
        heats = np.sort(self.paths.heat.unique())

        # Loop over heats and accumulate the heat delay then apply to the start and end times
        # of each igniter in the heat
        for i in heats:
            # We don't want to delay the first heat
            if i > 0:
                # Grab the dataframe rows for the current heat
                cur_heat = self.paths[self.paths.heat == i]

                # Accumulate time by multiplying the heat index by the heat delay
                time_diff = i*heat_delay

                # Loop over igniters in the heat and apply the time difference
                for idx, _ in cur_heat.iterrows():

                    # Adjust the start and time times
